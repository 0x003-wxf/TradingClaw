...
统一交易会话对上游只有一套语义，但底层建链不应与 `SECURITY` 或 `CRYPTO` 强绑定，而是由通道接入模式决定。
...
- `challenge_mode`：挑战模式，如 `NONE`、`IMAGE_CAPTCHA`、`SMS_OTP`
...
- 同一 `account_type=SECURITY` 也可以接签名型券商 API。
- 同一 `account_type=CRYPTO` 未来也可以接需要交互登录或 token 刷新的平台。
- 交易网关按 `provider_code + adapter_auth_mode + adapter_session_mode` 选择底层建链策略，而不是按资产类别硬编码登录方式。

统一要求：

- 无论底层是交互登录还是签名鉴权，对交易网关暴露的都必须是统一的 `TradingSession`。
- 交易网关不感知底层使用 `requests.Session`、cookie、token 还是签名请求，只消费适配域返回的会话可用性结果。

### 4.1.2 两类 `TradingSession` 建立方式

交互登录型 `TradingSession` 建立方式：

1. 交易网关根据 `provider_code + adapter_auth_mode=INTERACTIVE_LOGIN` 路由到目标适配域。
2. 适配域读取 `secret_schema=username_password_captcha_v1` 或同类凭据结构。
3. 若 `challenge_mode != NONE`，则先触发验证码或其他挑战获取。
4. 使用登录账号、密码和挑战结果完成认证。
5. 登录成功后，在适配域内建立运行时会话上下文，如 cookie、token 或 `requests.Session` 映射。
6. 适配域返回 `adapter_session_status=AVAILABLE` 后，交易网关把统一 `TradingSession` 推进为 `AVAILABLE`。

签名型 `TradingSession` 建立方式：

1. 交易网关根据 `provider_code + adapter_auth_mode=API_SIGNATURE` 路由到目标适配域。
2. 适配域读取 `secret_schema=api_key_secret_passphrase_v1` 或同类凭据结构。
3. 使用 `api_key + api_secret + passphrase` 或等价签名材料调用鉴权、账户或健康检查接口执行有效性校验。
4. 若权限、签名、时钟偏移和网络连通性均满足要求，则建立或刷新适配侧运行时上下文。
5. 适配域返回 `adapter_session_status=AVAILABLE` 后，交易网关把统一 `TradingSession` 推进为 `AVAILABLE`。

实现约束：

- 交互登录型的运行时会话对象可以是 `requests.Session` 或等价 HTTP 会话容器，但不得直接序列化后作为长期主存事实。
- 签名型适配不要求存在浏览器式 session，对交易网关而言“已完成凭据校验且具备稳定可调用能力”即可视作会话可用。
- 两类模式都必须在适配域落库存储最小必要会话元数据，如状态、过期时间、最近失败原因和引用 ID。
