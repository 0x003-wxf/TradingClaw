...
- 本文档覆盖 `identity-service` 与 `account-service`，定义身份认证、登录会话、账户绑定、账户能力投影和默认交易会话引用的统一设计。
...
2. 校验外部账户归属凭证、`credential_ref` 和账户类型所需的补充信息。
...
### 5.3.1 `credential_ref` 凭据模型

`credential_ref` 是账户域保存的“外部接入凭据引用”，并不等价于单一字段或单一登录方式。账户域只保存引用和元数据，真实敏感内容由密钥管理系统保存。

更合理的抽象方式不是把凭据模型直接绑定到账户类别，而是拆成三层：
...
| `account_type` | 账户类别，如 `SECURITY`、`CRYPTO` |
...
| `challenge_mode` | 是否存在验证码或二次挑战，如 `NONE`、`IMAGE_CAPTCHA`、`SMS_OTP` |
...
- 登录密码
- 验证码类型或验证码需求说明
- 可能存在的附加登录因子，如营业部、设备号、登录域标识

推荐 `secret_schema`：`username_password_captcha_v1`
...
推荐 `secret_schema`：`api_key_secret_passphrase_v1`

语义约束：

- 账户绑定接口传入的 `credential_ref` 只引用预先登记好的凭据，不直接上传明文敏感内容到业务接口。
- 账户域负责保存“这笔账户绑定关联哪份凭据引用”，不负责解释底层如何登录或如何签名。
